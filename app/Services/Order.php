...
class Order
{
    ...
    public function calculateFinalCosts()
    {
        ...
        if ($this->event->organiser->charge_tax == 1) {
            if ($this->event->organiser->tax_included==1) {
                $this->taxAmount = $this->orderTotalWithBookingFee - ($this->orderTotalWithBookingFee / 1.22);
                $this->orderTotalWithBookingFee -= $this->taxAmount;
            } else  {
    ...
    public function getVatFormattedInBrackets() {
        $prefix = "+";
        if ($this->event->organiser->tax_included) {
            $prefix = "inc.";
        }
        return "(".$prefix . $this->getTaxAmount(true) . " " . $this->event->organiser->tax_name . ")";
    }
